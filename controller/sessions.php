...
elseif(empty($_GET)){
	...
	if(!isset($jsondata->username) || !isset($jsondata->password)){
	    ...
	    $response->httpstatuscode(400);
	    $response->setsuccess(false);
	    (!isset($jsondata->username) ? $response->addmessage("username must be provided") : false);
	...
	if(strlen($jsondata->username) < 0 || strlen($jsondata->username) > 255 || strlen($jsondata->password) < 0 || strlen($jsondata->password) > 255){
		...
		(strlen($jsondata->username) > 255 ? $response->addmessage("username cannot exced 255 character") : false);
		(strlen($jsondata->password) < 0 ? $response->addmessage("username must not be empty") : false);
		(strlen($jsondata->password) > 255 ? $response->addmessage("username cannot exced 255 character") : false);
	    $response->send();
	    exit;
	}
